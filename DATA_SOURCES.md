...
| **Esri World Street Map / Dark Gray Canvas** (ArcGIS Online tile services) | The "Streets" and "Streets Dark" map stacks — Google/Apple-Maps-style street cartography (the dark canvas adds its Reference labels layer on top) | Same terms as Esri World Imagery above: keyless classic endpoints, attribution required, access governed by Esri | "Powered by Esri — Sources: Esri, HERE, Garmin, USGS, OpenStreetMap contributors, and the GIS User Community" |
...
| **Ontario 511 (511on.ca)** | CCTV camera catalog + frames, Ontario, Canada | Public Ontario Ministry of Transportation traffic camera data | "Ontario 511 (Ministry of Transportation) — 511on.ca" (courtesy) |
| **Waka Kotahi NZTA (trafficnz.info)** | CCTV camera catalog + frames, New Zealand | Public NZ Transport Agency traffic camera data | "Waka Kotahi NZ Transport Agency — trafficnz.info" (courtesy) |
| **Fintraffic / Digitraffic (digitraffic.fi)** | Weather/traffic camera catalog + frames, Finland | **CC BY 4.0** | "Fintraffic (Digitraffic) — digitraffic.fi" (attribution required) |
...
- **Ontario 511, NZ, & Finland traffic/weather cameras.** All three are keyless, nationwide/provincial government feeds (`511on.ca`, `trafficnz.info`, `digitraffic.fi`) added to broaden CCTV coverage beyond Austin/California/London. Ontario's per-site "Views" endpoint doubles as the direct JPEG frame URL; NZ's XML feed lists a direct `imageUrl` per camera; Finland's bulk station list omits the image URL, so it's built client-side from each preset's documented, stable `https://weathercam.digitraffic.fi/{presetId}.jpg` pattern rather than fetching a per-station detail endpoint for 800+ stations. All three packs can be disabled via `CCTV_ONTARIO_ENABLED`/`CCTV_NZ_ENABLED`/`CCTV_FI_ENABLED` and capped via their respective `_MAX_SOURCES` env vars.
