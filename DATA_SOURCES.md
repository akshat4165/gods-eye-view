...
- **Ontario 511 & NZ traffic cameras.** Both are keyless, nationwide/provincial government feeds (`511on.ca` and `trafficnz.info`) added to broaden CCTV coverage beyond Austin/California/London. Ontario's per-site "Views" endpoint doubles as the direct JPEG frame URL; NZ's XML feed lists a direct `imageUrl` per camera. Both packs can be disabled via `CCTV_ONTARIO_ENABLED`/`CCTV_NZ_ENABLED` and capped via their respective `_MAX_SOURCES` env vars.
